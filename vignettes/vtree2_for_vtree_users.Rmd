 ...
 * conditions for pruning are regular R expressions (e.g. `n < 20` to prune
   all nodes that have fewer than 20 samples)
...
I think that explicit splitting of the data before constructing the vtree
is more fitting. It is easy, and for complex cases there are many
...
character vector of labels, optionally with a custom format.
...
```{r summares_by_node,fig.height=3,fig.width=7}
...
mini-language are replaced by use of the glue formatting string or 
...
present, we need a more complex approach. In the example below, we will use
`summarize_by_node` (returning a data frame) and the `expr` argument to
`fmt_label` which uses a conditional formatting with the `glue` function.
...
```{r mtcars,fig.width=5,fig.height=8}
# mt <- mtcars
# mt$name <- rownames(mt)
# rownames(mt) <- NULL
#
 ...
 * `vtree_apply()`: partition some data by a vtree and call a function on
   each partition - conditional data analysis
 ...
 * ~~the summary expressions in the original vtree package are very powerful.
   ...
   in order.~~ -> introduced fmt as a glue expression plus added convenience
   variables such as `pct`, this is now more powerful than the original
   vtree syntax without a major hit on efficacy
 ...
 * use tidyverse principles for data manipulation: expressions as arguments
   evaluated in the context of a data mask corresponding to the vtree
   object
